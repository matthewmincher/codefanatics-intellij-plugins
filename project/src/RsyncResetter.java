 ...
 */
public class RsyncResetter extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        if(enabled){
            ...
            ProgressManager.getInstance().run(new Task.Backgroundable(project, "Removing Lockfile"){
                @Override
                public void run(@NotNull ProgressIndicator progressIndicator) {
                    progressIndicator.setFraction(0.0);

                    if(svc.reset != null && svc.reset.length() > 0){
                        try{
                            String command;
                            String[] args;

                            int space = svc.reset.indexOf(" ");
                            if(space > -1){
                                command = svc.reset.substring(0, space);

                                args = svc.reset.substring(space + 1).split(" ");
                            } else {
                                command = svc.reset;
                                args = new String[0];
                            }
                        ...
                        SwingUtilities.invokeLater(new Runnable() {
                            @Override
                            public void run() {
                                JBPopupFactory.getInstance().createHtmlTextBalloonBuilder("Rsync reset command is not configured", MessageType.ERROR, null)
                                        .setFadeoutTime(3000)
                                        .createBalloon()
                                        .show(RelativePoint.getCenterOf(statusBar.getComponent()), Balloon.Position.above);
                            }
